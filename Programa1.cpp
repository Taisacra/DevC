...
//INSERE PRODUTO NO ESTOQUE inserir no fim
...
//PROCURA UM DETERMINADO PRODUTO PELO CODIGO NO ESTOQUE 
Node* procurarProduto(Node* estoque){
	...
	while( produtoAtual->dados.codigo==cod){
		printf("\nCodigo: %d\n", produtoAtual->dados.codigo);
		printf("Nome: %s\n", produtoAtual->dados.nome);
        printf("Preco: %.2f\n", produtoAtual->dados.valor);
...
int cancelarVenda(PilhaVenda* pilhaVenda, Node* estoque) {
    if (estaVazia(pilhaVenda)) {
			printf("Nao ha venda para cancelar");
    }
            ...
            case 2:
                do {
                    ...
                    switch (opVenda) {
                        ...
                        case 4:
                            printf("Estoque atualizado pos venda");
                        	exibirEstoque(estoque);
                        	Sleep('15');
                            system("cls");
                            break;
